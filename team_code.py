...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    train_dataset = PCGDataset(data_folder, 
                               ...
                               target = 'Systolic murmur quality')
    ...
    quality_classifier = Hierachical_MS_Net(num_classes=DATASET_CFG['num_systolic_murmur_class'], **MODEL_CFG).to(device)
    optimizer = torch.optim.AdamW(quality_classifier.parameters(), **OPTIMIZER_CFG)
    ...
    # Stage 1: Train the classifier for quality classification
    if verbose:
        print('Training model for Systolic murmur quality classification...')
    for epoch in range(TRAINING_CFG['epochs']):
        if verbose:
            print(f'Epoch {epoch} starts...')
        train_epoch(train_loader, quality_classifier, optimizer, criterion, scheduler, device, TRAINING_CFG['print_freq'])
        if verbose:
            print('\n')
...
@torch.no_grad()
def recording_quality_diagnose(multi_scale_specs, quality_classifier, systolic_murmur_class, interval):
    quality_logits = quality_classifier(multi_scale_specs)
    quality_probs = F.softmax(quality_logits, -1).cpu().numpy()
    location_preds = calc_pred_locations(quality_probs, 
...
def run_challenge_model(model, data, recordings, verbose):
    ...
    recording_quality_preds = np.zeros(len(recordings), dtype=np.int_)

    for i in range(len(recordings)):
        multi_scale_specs, qualities = preprocessor(recordings[i], 4000, interval=interval)
        multi_scale_specs = [s.to(device) for s in multi_scale_specs]
        recording_quality_preds[i] = recording_quality_diagnose(multi_scale_specs, quality_classifier, systolic_murmur_class, interval)

    recording_quality_counts = np.bincount(recording_quality_preds, minlength=len(systolic_murmur_class))

    # Assign murmur labels based on the most frequent prediction
    quality_labels = np.zeros(len(systolic_murmur_class), dtype=np.int_)
    quality_labels[np.argmax(recording_quality_counts)] = 1
    classes=systolic_murmur_class
    probabilities = recording_quality_counts / np.sum(recording_quality_counts) if np.sum(recording_quality_counts) > 0 else np.zeros(len(systolic_murmur_class))

    return classes, quality_labels, probabilities
...
def save_challenge_model(model_folder, model, file_name='quality_classifier'):
    torch.save({
...
def load_challenge_model(model_folder, verbose):
    ...
    quality_checkpoint = torch.load(os.path.join(model_folder, 'quality_classifier.pth'), map_location=device)
    quality_classifier = Hierachical_MS_Net(num_classes=DATASET_CFG['num_systolic_murmur_class'], **MODEL_CFG).to(device)
    quality_classifier.load_state_dict(quality_checkpoint['model_state_dict'])
    ...
    return (device, preprocessor, quality_classifier, systolic_murmur_class)
