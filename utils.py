...
def convert_label_to_int(str_label):
    return quality_mapping_str2int[str_label]

def convert_label_to_str(int_label):
    return quality_mapping_int2str[int_label]

def get_quality_locations(data):
    locations = None
    for l in data.split('\n'):
...
def load_recordings_with_labels(data_folder, included_labels=['Blowing', 'Harsh']):  # Removed 'Unknown'
    ...
    for pf in patient_files:
        ...
        patient_quality = get_quality(patient_data)
        if patient_quality not in included_labels:
            continue
        patient_quality = included_labels.index(patient_quality)
        locations = get_locations(patient_data)
        quality_locations = get_quality_locations(patient_data)
        p_recordings = get_patient_recording_files(patient_data, len(locations))
        
        # Label recording as Blowing if quality is in the corresponding location, otherwise mark as harsh
        for i in range(len(locations)):
            if included_labels[patient_quality] == 'Blowing' and locations[i] not in quality_locations:
                continue
            else:
                patient_files_arr.append(pf)
                recording_files.append(os.path.join(data_folder, p_recordings[i]))
                qualities.append(patient_quality)

    patient_files_arr = np.array(patient_files_arr, dtype=np.str_)
    recording_files, qualities = np.array(recording_files, dtype=np.str_), np.array(qualities, dtype=np.int_)
    
    return patient_files_arr, recording_files, qualities
