...
# Get quality status from patient data.
def get_quality(data):
    qualitty = next((line.split(': ')[1].strip() for line in data.split('\n') if line.startswith('#Systolic murmur quality:')), None)
    if qualitty is None:
        raise ValueError('No qualitty available. Is your code trying to load labels from the hidden data?')
    return qualitty
...
def save_challenge_outputs(filename, patient_id, classes, labels, probabilities):
    ...
    expected_classes = ['Blowing', 'Harsh']
